...
//            DispatchQueue.global(qos: .background).async {
                if let imageUrl = photoData.imageUrl , let imageData = try? Data(contentsOf: imageUrl ) {
...
//                        let image = UIImage(data: imageData)!
                      cell.photoImageView.kf.setImage(with: imageUrl, placeholder: UIImage(named: "Placeholder"), options: [.keepCurrentImageWhileLoading])
...
//            }
          ...
          if let imageData = photoData.imageData, let imageUrl = photoData.imageUrl {
